...
class Settings{
		...
		add_action( 'admin_notices', function() {
			...
			<div class="notice notice-warning is-dismissible">
				<p>
					...
					<br>
					<?php printf( esc_html__( 'AWS S3 Console Access Id and Secret Key set ', 'next-payment' ), '<a href="' . admin_url( 'admin.php?page=theme-dev-aws-s3-settings' ) . '">', '</a>' ); ?>
				</p>
			</div>
			<?php
		} );
     ...
     */
     public function themeDev_aws_settings_css_loader(){
		...
		//echo $screen->id;
		if( in_array($screen->id, [ 'toplevel_page_theme-dev-aws-s3', 'wp-aws-s3_page_theme-dev-aws-s3-new-store', 'wp-aws-s3_page_theme-dev-aws-s3-upload-file', 'wp-aws-s3_page_theme-dev-aws-s3-settings']) ){
		...
		wp_register_style( 'themedev_ads', THE_DEV_AWS_PLUGIN_URL. 'assets/admin/css/ads.css');
