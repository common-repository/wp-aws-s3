...
class Awss3{
	...
	public function __construct(Themedevaws $newThemeAws, $custom_aws , $aws_text_domain){
		$_aws_credentials = get_option('theme-dev-aws-credentials', '');
		
		if( empty($_aws_credentials) || empty($_aws_credentials['aws_access_id'])){
			$this->_admin_notice();
			return;
		}
	...
	private function _admin_notice() {
		add_action( 'admin_notices', function() {
			?>
			<div class="notice notice-warning is-dismissible">
				<p>
					<strong><?php esc_html_e( 'AWS Access Id & AWS Secret Access Key  are missing.', 'next-payment' ); ?></strong>
					<br>
					<?php printf( esc_html__( '%s AWS S3 Console Access Id and Secret Key set %s', 'next-payment' ), '<a href="' . admin_url( 'admin.php?page=theme-dev-aws-s3-settings' ) . '">', '</a>' ); ?>
				</p>
			</div>
			<?php
		} );
	...
	public function themeDev_aws_s3_admin_new_store(){
		...
		if(isset($_POST['themedev-aws-create-store'])){
			...
			delete_transient('__aws_storename__');
	...
	public function themeDev_aws_s3_admin_upload_file(){
		...
		$file2 = __DIR__ .'/store/aws-storelist.json';
		$object_k = get_transient('__aws_storename__', '');

		if ( false === $object_k || empty( $object_k ) ) {
			$storeList = $this->s3Services->NX_bucket_list();
			if(isset($storeList['Buckets']) ){
				set_transient( '__aws_storename__', '' , 8640 );

				if(!empty($storeList['Buckets'])){
					file_put_contents($file2, 
						$this->__json($storeList['Buckets'])
					);
				}
			}
		}

		$storeList = $this->get_data( $file2, true);
	...
	public function get_data( $file, $type){
        if(file_exists($file)){
            return json_decode(
                    file_get_contents($file), $type
                );
        }
        return null;
    }
	...
	public function themeDev_aws_s3_admin_manage_store(){
		...
		$file = __DIR__ .'/store/aws-objectlist.json';
		$file2 = __DIR__ .'/store/aws-storelist.json';

		$object_k = get_transient('__aws_objectslist__', '');
		$object_store = get_transient('__aws_storename__', '');

		if(strlen($getBucketName) > 0){

			if ( false === $object_k || empty( $object_k )  || $object_store !== $getBucketName ) {
				
				$objcetList = $this->s3Services->NX_bucket_objects($getBucketName);	
				set_transient( '__aws_storename__', $getBucketName , 8640 );

				if( isset($objcetList['Contents']) ){
					set_transient( '__aws_objectslist__', 1 , 8640 );

					if(!empty($objcetList['Contents'])){
						file_put_contents($file, 
							$this->__json($objcetList['Contents'])
						);
					}
				}
		...
		if( isset($_GET['store']) ){
			$objcetList = $this->get_data( $file, true);
