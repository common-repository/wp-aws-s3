<section class="<?php echo esc_attr('themeDev-aws-body');?>">
	...
	<button onclick="nxaws_go_back()" class="aws-back-button"> <span class="dashicons dashicons-arrow-left-alt2"></span> </button>
	<button onclick="nxaws_go_forword()"  class="aws-back-button"> <span class="dashicons dashicons-arrow-right-alt2"></span> </button>

	<div class="<?php echo esc_attr('themeDev-aws-form');?>">
		<ul class="manage-aws-themeDev">
		<?php
			if(isset($storeList) && !empty($storeList) ):
				$m = 1;
				foreach($storeList AS $storeListData):
...
//echo '<pre>'; print_r($objcetList); echo '</pre>';

if(isset($objcetList) && !empty($objcetList) ):
	$filearray=[];
	foreach($objcetList as $fileentry) {
	...
	$fileList = is_array($filearray) ? $filearray : '';
